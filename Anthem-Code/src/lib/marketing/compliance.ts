...
export function assertSourceUrl(
  ...
  if (!opts?.internal && /\/admin\//i.test(trimmed)) {
    throw new Error("admin deep link ต้องระบุ lead_origin=internal");
  }
  return trimmed;
}

export function isInternalLead(lead: { lead_origin?: string | null; tags?: string[] | null }): boolean {
  return lead.lead_origin === "internal" || (lead.tags ?? []).includes("internal");
}

export function redactLeadForExport(lead: {
  source_url: string;
  lead_origin?: string | null;
  tags?: string[] | null;
}): string {
  if (isInternalLead(lead)) return "[admin internal link]";
