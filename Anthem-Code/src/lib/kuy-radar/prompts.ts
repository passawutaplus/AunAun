import type { KuyAiOutput, KuyAiTask, KuyInsightType } from "./types";

const COMPLIANCE_NOTE =
  "ข้อมูลช่วยตัดสินใจ ไม่ใช่ข้อเท็จจริง 100% — ตรวจสอบ source URL และสิทธิ์ข้อมูลก่อนใช้งาน";

export function buildPrompt(task: KuyAiTask, context: Record<string, string>): string {
...
export function mockAiOutput(task: KuyAiTask, insightType?: KuyInsightType): KuyAiOutput {
  const byTask: Partial<Record<KuyAiTask, KuyAiOutput>> = {
    analyze_lead_intent: {
...
export async function runKuyAiTask(
  task: KuyAiTask,
  context: Record<string, string>,
  insightType?: KuyInsightType,
): Promise<KuyAiOutput> {
  const useMock = import.meta.env.VITE_KUY_RADAR_AI_MOCK !== "false";
