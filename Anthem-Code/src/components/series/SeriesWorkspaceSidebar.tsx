    ...
    >
      <div className="px-1 py-2 pr-3">
        <div className="min-w-0">
          <p className="text-sm font-semibold text-foreground truncate">Catalog</p>
          <p className="text-[11px] text-muted-foreground tabular-nums">
            {series.length} Catalog · {totalItems} ชิ้น
          </p>
        </div>
      </div>
      ...
      <div className="pr-3 pb-2">
        <div className="relative">
          ...
          <Input
            ...
            placeholder="ค้นหา Catalog หรือผลงาน…"
            className="h-9 rounded-xl border-border/40 bg-transparent pl-8 text-sm"
          />
        </div>
      </div>

      <nav className="min-h-0 flex-1 overflow-y-auto pr-3 pb-3 space-y-0.5" aria-label="รายการ Catalog">
        {!isSearching && (
          <>
            <button
              type="button"
        ...
        {noSearchResults ? (
          <p className="px-2 py-6 text-center text-xs text-muted-foreground">
            ไม่พบ Catalog หรือผลงานที่ตรงกับคำค้น
          </p>
        ) : (
          <>
