...
/** Full My Catalog workspace (sidebar + folders/works). */
export default function CatalogManagePanel({ userId, embedded }: Props) {
  const navigate = useNavigate();
  const [params, setParams] = useSearchParams();
  const { data: series = [], isLoading, isError, refetch } = useMyProjectSeries(userId);
  ...
  const handleDropProject = async (seriesId: string, projectId: string) => {
    try {
      ...
      toast.success("เพิ่มผลงานเข้า Catalog แล้ว");
      if (selection !== seriesId) select(seriesId);
    } catch (e: unknown) {
      toast.error(e instanceof Error ? e.message : "เพิ่มเข้า Catalog ไม่สำเร็จ");
    }
  };
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {selection === "works"
              ? "ดูผลงานทั้งหมด — ลากการ์ดไปวางที่โฟลเดอร์ Catalog ทางซ้ายได้"
      ...
      <div>
        {isLoading ? (
          ...
          <div className="text-center py-16 glass-panel rounded-2xl space-y-3">
            <p className="text-foreground font-medium">โหลด Catalog ไม่สำเร็จ</p>
          ...
          <div className="flex gap-4 lg:gap-5 items-start">
            <SeriesWorkspaceSidebar
              ...
              className="hidden lg:flex w-64 xl:w-72 shrink-0 sticky top-4 h-[calc(100vh-8rem)] pr-1"
            />
              ...
              <FeedModeTransition modeKey={selection}>
                {selection === "works" ? (
                  ...
                  <div className="rounded-2xl border border-dashed border-border/70 bg-muted/20 px-4 py-10 text-center text-sm text-muted-foreground">
                    ไม่พบ Catalog ตามตัวกรอง — ลองเปลี่ยนสถานะหรือคำค้นหา
                  </div>
                ) : (
      ...
      <Sheet open={mobileNavOpen} onOpenChange={setMobileNavOpen}>
        <SheetContent side="left" className="w-[min(100%,20rem)] p-0 border-border/60">
          <SheetHeader className="sr-only">
            <SheetTitle>รายการ Catalog</SheetTitle>
          </SheetHeader>
          <SeriesWorkspaceSidebar
            series={series}
            projects={projects}
