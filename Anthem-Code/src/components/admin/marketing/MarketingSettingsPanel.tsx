...
export default function MarketingSettingsPanel() {
  ...
  const { activeBusinessId, deleteBusinessData } = useMarketingBusinesses();
  const { settings, saveSettings } = useMarketingSettings(activeBusinessId);
  const [form, setForm] = useState({
    default_language: "both" as MarketingLanguage,
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          <label className="text-sm">
            ...
            <select
              ...
              onChange={(e) => setForm((f) => ({ ...f, default_language: e.target.value as MarketingLanguage }))}
            >
