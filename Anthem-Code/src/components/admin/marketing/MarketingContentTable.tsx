...
export default function MarketingContentTable() {
  const { activeBusinessId } = useMarketingBusinesses();
  const { contentItems, createContent } = useMarketingContent(activeBusinessId);
  ...
  const add = async () => {
    try {
      ...
      setForm({ title: "", platform: MARKETING_PLATFORMS[0], content_url: "", hook: "" });
    } catch (e) {
            ...
            {contentItems.map((c) => (
              <tr key={c.id} className="border-b border-admin-border">
                ...
                <td className="px-4 py-3">
                  <a href={c.content_url} target="_blank" rel="noopener noreferrer" className="marketing-link inline-flex gap-1">
