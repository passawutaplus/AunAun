...
export default function MarketingCompetitorTable() {
  const { activeBusinessId } = useMarketingBusinesses();
  const { competitors, createCompetitor } = useMarketingCompetitors(activeBusinessId);
  ...
  const add = async () => {
    try {
      ...
      setForm({ competitor_name: "", platform: MARKETING_PLATFORMS[0], profile_url: "", main_offer: "", threat_level: "medium" });
    } catch (e) {
  ...
  return (
    <div className="space-y-4">
      <MarketingCard className="p-5">
        ...
        <button type="button" onClick={() => void add()} className="marketing-btn-primary mt-3 rounded-lg px-4 py-2 text-sm">
          Save
        </button>
      </MarketingCard>
            ...
            {competitors.map((c) => (
              <tr key={c.id} className="border-b border-admin-border">
                ...
                <td className="px-4 py-3">
                  <a href={c.profile_url} target="_blank" rel="noopener noreferrer" className="marketing-link inline-flex gap-1">
