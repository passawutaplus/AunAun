...
import { parseLeadsCsv, useMarketingLeads } from "@/hooks/admin/useMarketingLeads";
import { MARKETING_LEAD_STATUSES, MARKETING_PLATFORMS, type MarketingLeadStatus } from "@/lib/marketing/types";
import { isInternalLead } from "@/lib/marketing/compliance";
import { marketingStatusLabel } from "@/lib/marketing/i18n";
import { MarketingCard } from "./MarketingShell";

export default function MarketingLeadTable() {
  const { uiLanguage } = useMarketingContext();
  const { activeBusinessId } = useMarketingBusinesses();
  const { leads, createLead, bulkUpdateStatus } = useMarketingLeads(activeBusinessId);
  ...
  const filtered = useMemo(() => {
    ...
    return leads.filter((l) => {
      ...
      const internal = isInternalLead(l);
      const matchOrigin =
        origin === "all" ||
      ...
      return matchPlatform && matchOrigin && matchQuery;
    });
  }, [leads, platform, origin, query]);
    ...
    <MarketingCard className="overflow-hidden">
      <div className="border-b border-admin-border p-5">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="marketing-section-label text-xs font-semibold uppercase tracking-wide">Lead Finder</p>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <label className="relative block">
              ...
              <input
                ...
                className="marketing-input h-9 w-56 rounded-lg pl-9 pr-3 text-sm"
              />
            </label>
            <select
              className="h-9 rounded-lg border border-admin-border px-2 text-sm"
              value={origin}
              onChange={(e) => setOrigin(e.target.value as "all" | "internal" | "external")}
            >
              <option value="all">{uiLanguage === "th" ? "ทั้งหมด" : "All"}</option>
              <option value="internal">{uiLanguage === "th" ? "In-app" : "In-app"}</option>
              <option value="external">{uiLanguage === "th" ? "External" : "External"}</option>
            </select>
            <select
              className="h-9 rounded-lg border border-admin-border px-2 text-sm"
              value={platform}
        ...
        {selected.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-2">
            {MARKETING_LEAD_STATUSES.slice(0, 4).map((status) => (
              <button
                key={status}
                type="button"
              ...
              >
                → {marketingStatusLabel(uiLanguage, status)}
              </button>
            ))}
          </div>
        )}
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[820px] text-left text-sm">
          ...
          <tbody>
            {filtered.map((lead) => (
              <tr key={lead.id} className="marketing-row-hover border-b border-admin-border">
                ...
                <td className="px-4 py-3">{marketingStatusLabel(uiLanguage, lead.status)}</td>
                <td className="px-4 py-3">
                  <a
                    href={lead.source_url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="marketing-link inline-flex items-center gap-1 text-xs font-semibold"
                  >
                    Open source
        ...
        <div className="border-t border-admin-border p-5">
          ...
          <div className="mt-2 flex gap-2">
            <button
              ...
              className="marketing-btn-primary rounded-lg px-4 py-2 text-sm"
            >
              Import
            </button>
