...
export function KuyComplianceGuard({ onExport, disabled }: Props) {
  ...
  return (
    <KuyRadarCard className="p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="kuy-section-label text-xs font-semibold uppercase tracking-wide">Compliance Guard</p>
      ...
      <label className="kuy-callout-warn mt-4 flex cursor-pointer items-start gap-3 rounded-lg p-3 text-sm">
        <input
          type="checkbox"
          checked={exportReady}
        ...
        />
        <span className="leading-5 text-admin-fg">{kuyT(uiLanguage, "exportConfirm")}</span>
      </label>
      <div className="mt-4 grid gap-2 sm:grid-cols-3">
        {(["csv", "xlsx", "pdf"] as const).map((format) => (
          <button
            ...
            className="kuy-btn-primary inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-medium shadow-sm transition disabled:cursor-not-allowed disabled:opacity-45"
          >
