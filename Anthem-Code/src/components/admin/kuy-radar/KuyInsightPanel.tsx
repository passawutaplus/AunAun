...
import type { KuyInsightType } from "@/lib/kuy-radar/types";
import { KuyRadarCard } from "./KuyRadarShell";

const tabs: { id: KuyInsightType; label: string }[] = [
...
export default function KuyInsightPanel() {
  const { uiLanguage } = useKuyRadarContext();
  const { activeBusiness, activeBusinessId } = useKuyRadarBusinesses();
  const { insights, runInsight, isRunning } = useKuyRadarInsights(activeBusinessId);
  const [tab, setTab] = useState<KuyInsightType>("customer");
    ...
    <div className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
      <KuyRadarCard className="p-5">
            ...
            {tabs.map((t) => (
              <button
                ...
                className={`rounded-md px-3 py-2 text-xs font-semibold capitalize ${
                  tab === t.id ? "kuy-nav-link-active" : "kuy-nav-link"
                }`}
              >
              {t.label}
            </button>
        ...
        <button
          ...
          disabled={isRunning}
          onClick={() => void run()}
          className="kuy-btn-primary inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm disabled:opacity-60"
        >
          <Wand2 className="h-4 w-4" />
          {isRunning ? "Running…" : "Run task"}
        </button>
        <p className="mt-4 text-xs text-amber-800">{kuyT(uiLanguage, "aiDisclaimer")}</p>
      </KuyRadarCard>
      <KuyRadarCard className="p-5">
        {latest ? (
          <div className="space-y-3">
            ...
            {latest.recommendation && (
              <div className="kuy-callout rounded-lg p-4 text-sm">{latest.recommendation}</div>
            )}
          </div>
        ) : (
