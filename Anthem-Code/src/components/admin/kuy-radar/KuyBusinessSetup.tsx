...
import { KUY_APLUS1_DEFAULT_BUSINESS } from "@/lib/kuy-radar/aplus1";
import { KUY_BUSINESS_PRESETS, KUY_PLATFORMS, type KuyLanguage } from "@/lib/kuy-radar/types";
import { KuyRadarCard } from "./KuyRadarShell";

const defaultForm = () => ({
  business_name: KUY_APLUS1_DEFAULT_BUSINESS.business_name,
  category: KUY_BUSINESS_PRESETS[0],
  ...
  language: "both" as KuyLanguage,
  main_keyword: KUY_APLUS1_DEFAULT_BUSINESS.main_keyword,
  pain_points: KUY_APLUS1_DEFAULT_BUSINESS.pain_points.join(", "),
  goals: KUY_APLUS1_DEFAULT_BUSINESS.goals.join(", "),
  preferred_platforms: [...KUY_APLUS1_DEFAULT_BUSINESS.preferred_platforms],
});

export default function KuyBusinessSetup() {
  const { uiLanguage, setLanguagePref } = useKuyRadarContext();
  const { activeBusiness, createBusiness, updateBusiness, isSaving } = useKuyRadarBusinesses();
  ...
  const applyPreset = (category: string) => {
    if (category === KUY_BUSINESS_PRESETS[0]) {
  ...
  const inputClass = "kuy-input mt-1 w-full rounded-lg px-3 py-2 text-sm";
    ...
    <KuyRadarCard className="p-5">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        ...
        <label className="text-sm">
          ...
          <select className={inputClass} value={form.language} onChange={(e) => setForm((f) => ({ ...f, language: e.target.value as KuyLanguage }))}>
        ...
        {KUY_PLATFORMS.map((p) => (
          <button
            ...
            className={`rounded-full px-3 py-1.5 text-xs font-medium ${
              form.preferred_platforms.includes(p) ? "kuy-pill-on" : "kuy-pill-off"
            }`}
          >
            {p}
          </button>
      ...
      <button
        ...
        disabled={isSaving}
        onClick={() => void save()}
        className="kuy-btn-primary mt-6 rounded-lg px-4 py-2 text-sm font-medium disabled:opacity-60"
      >
