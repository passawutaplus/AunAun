...
import {
  averagePackageOverviewMetric,
  buildPackageOverviewSeries,
  PACKAGE_OVERVIEW_METRICS,
  packageOverviewMetricConfig,
  sumPackageOverviewMetric,
  viewSeriesTrendPercent,
  type PackageOverviewMetric,
} from "@/lib/packageOverviewSeries";
...
export default function PackageOverviewChart({ ownerId, serviceIds }: Props) {
  const [dateRange, setDateRange] = useState<ProjectStatsDateRange>(DEFAULT_PROJECT_STATS_DATE_RANGE);
  const [metric, setMetric] = useState<PackageOverviewMetric>("views");
  ...
  const metricConfig = packageOverviewMetricConfig(metric);

  const { data, isLoading, isFetching } = usePackageOverviewSeries(
    ownerId,
    serviceIds,
  ...
  const series = useMemo(() => {
    ...
    return buildPackageOverviewSeries(data.current, bounds.from, bounds.to, granularity);
  }, [bounds, data, granularity]);

  const total = sumPackageOverviewMetric(series, metric);
  const average = averagePackageOverviewMetric(series, metric);
  ...
  return (
    <section className="rounded-2xl border border-border/70 bg-card/40 p-4 space-y-3">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="text-[11px] text-muted-foreground mt-0.5">
            ช่วง {rangeLabel} · ทุกแพ็กเกจรวมกัน
          </p>
        </div>
      ...
      <div className="flex gap-1.5 overflow-x-auto pb-0.5 scrollbar-hide">
        {PACKAGE_OVERVIEW_METRICS.map((item) => (
          <button
            key={item.id}
            type="button"
      ...
      <div className="h-44 w-full">
        {loading ? (
          ...
          <div className="h-full flex items-center justify-center text-xs text-muted-foreground text-center px-4">
            ยังไม่มีข้อมูลในช่วงนี้ — เมื่อมีคนเปิดดูหรือกดจ้างจากแพ็กเกจ จะโชว์ที่นี่
          </div>
        ) : (
          <ChartContainer config={chartConfig} className="h-full w-full aspect-auto">
            <AreaChart data={series} margin={{ top: 4, right: 8, left: -18, bottom: 0 }}>
              <defs>
                <linearGradient id="packageOverviewFill" x1="0" y1="0" x2="0" y2="1">
