...
DROP FUNCTION IF EXISTS anthem.auto_grant_admin() CASCADE;
...
-- Authenticated must not share the broad active-row policy (PII grants would leak).
...
GRANT SELECT (
  email, phone, tax_id, bank_account_number, payment_qr_url, stripe_connect_account_id,
  address, onboarding_visits, account_status, is_active, tester_approved
) ON public.profiles TO authenticated;
...
-- Append: revoke anon money surface + restore wallet owner grants
-- (applied live 2026-07-22 during red-team probe)

-- See migrations applied via Supabase MCP:
-- security_p0_revoke_anon_money_surface
-- security_p0_harden_client_job_fulfill
-- security_p0_restore_wallet_owner_grants
