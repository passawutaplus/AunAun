-- KYC standard v3: DOB/age, nationality, PEP declaration, submission meta, re-KYC expiry
-- Apply via: node scripts/ecosystem/apply-kyc-migration.mjs (or supabase SQL editor)
...
ALTER TABLE shared.kyc_requests
  ...
  ADD COLUMN IF NOT EXISTS submission_meta jsonb NOT NULL DEFAULT '{}'::jsonb,
...
-- Drop prior overloads so we can redefine with new args
DROP FUNCTION IF EXISTS public.submit_kyc_verification(text, text, text, text, text, jsonb, text);
DROP FUNCTION IF EXISTS public.submit_kyc_verification(text, text, text, text, text, jsonb, text, text, text, text, jsonb);
