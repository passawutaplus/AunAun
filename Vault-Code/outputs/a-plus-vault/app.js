...
import { createMoodboardDialogMarkup, linkProjectDialogMarkup, moodboardListMarkup, moodboardVaultPickerMarkup, moodboardColorChooserMarkup, smartGridEditorMarkup } from "./modules/moodboard-ui.js";
...
import { computeDashboardStats, settingsOverviewMarkup } from "./modules/user-dashboard.js";
let state={user:load(S.user,null),items:normalizeItems(load(S.items,SEED)),cols:ensureCoreCols(normalizeCols(load(S.cols,DEFAULT_COLS))),projects:null,moodboards:normalizeMoodboards(load(S.moodboards,[])),view:"vault",type:"all",col:"all",q:"",searchOpen:false,theme:load(S.theme,"system"),selected:null,selectedIds:[],modal:false,mode:"image",activeProject:"kitchen",activeBoard:"warm",activeMoodboard:null,selectedObject:null,selectedObjectIds:[],leftCollapsed:true,rightCollapsed:false,rightWidth:clampRightWidth(load(S.rightWidth,380)),openMenu:null,toast:"",collectionPicker:null,dialog:null,sortBy:"saved_new",sortMenu:false,libraryView:normalizeLibraryView(load(S.libraryView,"medium")),viewMenu:false,filterColor:"all",filterStyle:"all",filterCategory:"all",filterKeyword:"",filterHex:"",loading:null,animateVault:false,publicObject:null,drawerAnimating:false,mediaLightbox:null,moodboardSaveStatus:"idle",moodboardSourceCollapsed:false,moodboardInspectorCollapsed:false,moodboardSourceWidth:clamp(Number(load(S.moodboardSourceWidth,240))||240,180,420),moodboardInspectorWidth:clamp(Number(load(S.moodboardInspectorWidth,260))||260,200,420),moodboardTool:"select",moodboardConnectFrom:null};
...
normalizeVaultEntry();vaultRemote.consumeAuthCallback?.();importDeepLinkCapture();applyTheme({instant:true});render();initRemoteSession();startDevAutoRefresh();syncExtensionCaptures(true);setInterval(()=>syncExtensionCaptures(true),4500);
...
document.addEventListener("click",e=>{let toggle=e.target&&e.target.closest?e.target.closest("[data-sorttoggle]"):null;if(toggle){e.preventDefault();e.stopPropagation();state.sortMenu=!state.sortMenu;state.viewMenu=false;render();return}let option=e.target&&e.target.closest?e.target.closest("[data-sortby]"):null;if(option){e.preventDefault();e.stopPropagation();state.sortBy=option.dataset.sortby;state.sortMenu=false;render();return}let color=e.target&&e.target.closest?e.target.closest("[data-filtercolor]"):null;if(color){e.preventDefault();e.stopPropagation();state.filterColor=color.dataset.filtercolor||"all";render();return}let style=e.target&&e.target.closest?e.target.closest("[data-filterstyle]"):null;if(style){e.preventDefault();e.stopPropagation();state.filterStyle=style.dataset.filterstyle||"all";render();return}let category=e.target&&e.target.closest?e.target.closest("[data-filtercategory]"):null;if(category){e.preventDefault();e.stopPropagation();state.filterCategory=category.dataset.filtercategory||"all";render();return}let clear=e.target&&e.target.closest?e.target.closest("[data-clearfilters]"):null;if(clear){e.preventDefault();e.stopPropagation();state.filterColor="all";state.filterStyle="all";state.filterCategory="all";state.filterKeyword="";state.filterHex="";render();return}let viewToggle=e.target&&e.target.closest?e.target.closest("[data-viewtoggle]"):null;if(viewToggle){e.preventDefault();e.stopPropagation();state.viewMenu=!state.viewMenu;state.sortMenu=false;render();return}let viewOpt=e.target&&e.target.closest?e.target.closest("[data-library-view]"):null;if(viewOpt){e.preventDefault();e.stopPropagation();setLibraryView(viewOpt.dataset.libraryView);return}if(state.sortMenu&&e.target&&e.target.closest&&!e.target.closest(".vault-sort")){state.sortMenu=false;render()}if(state.viewMenu&&e.target&&e.target.closest&&!e.target.closest(".vault-view")){state.viewMenu=false;render()}},true);
...
function createCollection(name,opts){opts=opts||{};let c={id:id(),name:name.trim(),system:false,parentId:"",sortOrder:nextCollectionSortOrder(""),pinnedAt:0};state.cols=state.cols.concat(c);save(S.cols,state.cols);syncRemoteCollection(c,"create");if(!opts.keepView){state.col=c.id;state.view="vault"}if(!opts.skipToast)toast(opts.toastMessage||"Collection created.");return c}
...
function headerProfileButton(){if(!state.user)return"";return "<button type='button' class='header-profile-button' data-view='profile' title='Profile settings' aria-label='Profile settings'>"+profileAvatarMarkup("header-avatar")+"</button>"}
function shell(inner){return "<div class='app-shell"+(!state.user?" needs-auth":"")+"'><header class='topbar'><div class='topbar-logo'>"+brandMark()+"</div><div class='topbar-spacer' aria-hidden='true'></div><div class='header-actions'>"+headerSearchMarkup()+"<button class='save-button upload-plus-button' data-open title='Upload to Vault' aria-label='Upload to Vault'><span class='upload-plus-text'>+</span></button>"+headerProfileButton()+"</div></header>"+inner+(state.modal?modal():"")+(state.dialog?appDialog():"")+(state.mediaLightbox?mediaLightboxMarkup():"")+(!state.user?authGateMarkup():"")+(state.toast?"<div class='toast'>"+esc(state.toast)+"</div>":"")+selectionActionsMarkup()+"<button type='button' class='back-to-top' data-back-top hidden title='Back to top' aria-label='Back to top'><svg class='back-to-top-icon' viewBox='0 0 24 24' aria-hidden='true' fill='none' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><path d='M12 19V5'/><path d='m5 12 7-7 7 7'/></svg></button></div>"}
...
function sideNav(statsHtml){let views=[["vault","vault","Vault Library"],["moodboards","board","Moodboard"],["projects","project","Projects"],["collections","collections","Collections"]],collapse=railCollapseButton(),header=state.leftCollapsed?"<div class='side-shell-header is-collapsed-header' aria-hidden='true'></div>":"<div class='side-shell-header is-expanded-header'>"+collapse+"</div>";return header+(statsHtml||"")+"<div class='side-section sidebar-workspace'><p class='side-kicker'>Workspace</p><div class='side-nav'>"+views.map(v=>{let active=v[0]==="moodboards"?(state.view==="moodboards"||state.view==="moodboard-edit"||state.view==="board"):v[0]==="projects"?(state.view==="projects"||state.view==="project"):v[0]==="collections"?state.view==="collections":state.view===v[0];return "<button class='side-nav-button "+(active?"active":"")+"' data-view='"+v[0]+"' title='"+v[2]+"'><span class='side-icon'>"+icon(v[1])+"</span><span class='side-label'>"+v[2]+"</span></button>"}).join("")+"</div></div>"+sidebarUploadZone()+(state.leftCollapsed?"<div class='sidebar-rail-footer'>"+collapse+"</div>":"")}
...
function libraryViewLabel(mode){return mode==="small"?"Small icons":mode==="large"?"Extra large icons":mode==="details"?"Details":"Medium icons"}
...
function moodboardsView(){let stats=count(),realCols=state.cols.filter(c=>!c.system),cls="workspace overview-workspace detail-closed"+(state.leftCollapsed?" left-collapsed":"");return shell("<div class='"+cls+"'><aside class='rail'>"+sideNav(vaultStatsBlock(stats,realCols))+sidebarMain()+"</aside><main class='main overview-main'>"+moodboardListMarkup({moodboards:state.moodboards,projects:state.projects,esc,escA,icon,emptyPrimary:"Open Vault Library"})+"</main></div>")}
function moodboardEditView(){let board=activeMoodboard();if(!board){state.view="moodboards";return moodboardsView()}ensureMoodboardAutosave();return shell(smartGridEditorMarkup({board,items:state.items,esc,escA,icon,uiIcon,media,host,saveStatus:state.moodboardSaveStatus,canUndo:moodboardHistory.canUndo(),canRedo:moodboardHistory.canRedo(),selectedObjectId:state.selectedObject,selectedObjectIds:state.selectedObjectIds||[],tool:state.moodboardTool||"select",sourceCollapsed:!!state.moodboardSourceCollapsed,inspectorCollapsed:!!state.moodboardInspectorCollapsed,sourceWidth:state.moodboardSourceWidth,inspectorWidth:state.moodboardInspectorWidth}))}
...
function profileView(){let stats=count(),realCols=state.cols.filter(c=>!c.system),s=storageBreakdown(),email=state.user&&state.user.email||"creative@aplus.local",displayName=(state.user&&state.user.displayName||"").trim(),favoriteStyles=Array.isArray(state.user&&state.user.favoriteStyles)?state.user.favoriteStyles.join(", "):String(state.user&&state.user.favoriteStyles||""),site=(window.APLUS_VAULT_RUNTIME&&window.APLUS_VAULT_RUNTIME.siteUrl)||location.origin,dashboard=computeDashboardStats(state,{storageBreakdown,formatBytes,getVaultApiToken}),overview=settingsOverviewMarkup(dashboard,{esc,icon,formatBytes});return shell("<div class='workspace profile-workspace"+(state.leftCollapsed?" left-collapsed":"")+" detail-closed'><aside class='rail'>"+sideNav(vaultStatsBlock(stats,realCols))+sidebarMain()+"</aside><main class='main profile-main'><section class='settings-page'><header class='settings-page-head'><span class='section-label'>Account</span><h1>Settings</h1><p>Overview, profile, appearance, extension sync, storage, and your Vault data.</p></header><div class='settings-layout'><section class='settings-panel'>"+overview+"<article class='settings-card settings-card-primary'><div class='settings-card-head'><h2>Profile</h2><p>Your name, login, and creative style preferences.</p></div><form class='profile-form settings-form' data-profile><div class='profile-avatar-editor'><div class='profile-avatar-preview'>"+profileAvatarMarkup("settings-avatar")+"</div><div class='profile-avatar-actions'><label class='ghost-button profile-avatar-upload'>"+icon("image")+"<span>"+(state.user&&state.user.avatarUrl?"Change photo":"Upload photo")+"</span><input type='file' data-avatar-upload accept='image/jpeg,image/png,image/webp' hidden></label>"+(state.user&&state.user.avatarUrl?"<button type='button' class='ghost-button' data-avatar-remove>Remove photo</button>":"")+"<p class='settings-field-hint'>JPG, PNG, or WebP up to 2MB. Shown as a circle in the header.</p></div></div><label>Display name<input name='displayName' type='text' value='"+escA(displayName)+"' placeholder='Your name' autocomplete='name'></label><label>Email<input name='email' type='email' value='"+escA(email)+"' autocomplete='username'></label><label>Favorite styles<input name='favoriteStyles' type='text' value='"+escA(favoriteStyles)+"' placeholder='minimal, branding, campaign'></label><p class='settings-field-hint'>Comma-separated styles you save most often.</p><label>Password<input name='password' type='password' value='aplusvault' autocomplete='current-password'></label><div class='settings-actions'><button class='primary-button'>Save profile</button><button type='button' class='google-button profile-google' data-google-login>"+icon("google")+"<span>Continue with Google</span></button></div></form><button type='button' class='ghost-button wide settings-logout' data-logout>Log out</button></article><article class='settings-card'><div class='settings-card-head'><h2>Appearance</h2><p>Choose light, dark, or match your system.</p></div><div class='settings-theme-row'>"+themeControl()+"</div></article><article class='settings-card'><div class='settings-card-head'><h2>Extension sync</h2><p>Copy this token into the Chrome extension under <strong>Alpha API Token</strong>. Set API Base to <code>"+esc(site.replace(/\/$/,""))+"</code>.</p></div><div class='extension-token-box'><code data-extension-token>"+esc(getVaultApiToken()||"Log in to generate a token")+"</code></div><div class='settings-actions'><button type='button' class='primary-button' data-copy-extension-token>Copy token</button><button type='button' class='ghost-button' data-regenerate-extension-token>Refresh token</button></div></article></section><aside class='settings-aside'><article class='settings-card storage-card'><div class='settings-card-head'><h2>Storage</h2><p>1 GB Vault quota for references and uploads.</p></div><div class='storage-meter'><strong>"+formatBytes(s.total)+"</strong><span>of 1 GB used</span><i><b style='width:"+Math.min(100,s.total/s.limit*100).toFixed(2)+"%'></b></i></div><div class='storage-breakdown'>"+storageRows(s).join("")+"</div></article><article class='settings-card privacy-card'><div class='settings-card-head'><h2>Privacy & Legal</h2><p>Your references are private by default. Export, delete, or manage your Vault data here.</p></div><div class='settings-stack privacy-data-actions'><button type='button' class='primary-button wide' data-export-vault>Export my data</button><button type='button' class='ghost-button wide' data-clear-vault>Clear local Vault data</button><button type='button' class='ghost-button wide danger-button' data-delete-account>Delete account</button></div><div class='legal-link-list'><a href='./legal.html#privacy' target='_blank' rel='noreferrer'>Privacy Notice</a><a href='./legal.html#data-rights' target='_blank' rel='noreferrer'>Export & Deletion</a><a href='./legal.html#copyright' target='_blank' rel='noreferrer'>Copyright Report</a><a href='./legal.html#ai' target='_blank' rel='noreferrer'>AI Processing</a><a href='./legal.html#security' target='_blank' rel='noreferrer'>Security</a></div></article></aside></div></section></main></div>")}
function brandMark(){return "<div class='brand-mark header-logo' aria-hidden='true'>A+</div>"}
function brand(options){let sidebar=options&&options.sidebar;return "<div class='brand"+(sidebar?" sidebar-brand":"")+"'><div class='brand-mark'>A+</div>"+(sidebar?"<div><p class='brand-title'>A+ Vault</p><p class='brand-subtitle'>You Create, We Connect</p></div>":"")+"</div>"}
...
function importDeepLinkCapture(){normalizeVaultEntry();let objectId=readDeepLinkObjectId();if(objectId){syncObjectDeepLink(objectId);return}let hash=location.hash||"",prefix="#vault-capture=";if(!hash.startsWith(prefix))return;try{let payload=decodeVaultPayload(hash.slice(prefix.length)),key="hash:"+hash.slice(prefix.length,161),seen=load(S.captures,[]);if(!seen.includes(key)){let item=itemFromCapturePayload(payload);state.items=[item].concat(state.items);state.selected=item.id;state.sortBy="saved_new";save(S.items,state.items);save(S.captures,seen.concat(key).slice(-500));syncRemoteItem(item,"create")}state.view="vault";history.replaceState(null,"",location.origin+"/vault#object="+encodeURIComponent(state.selected||""))}catch(e){console.warn("Could not import Vault capture.",e)}}
...
function filterVaultByKeyword(keyword){let tag=String(keyword||"").trim();if(!tag)return;state.filterKeyword=tag;state.filterHex="";state.q="";state.col="all";state.view="vault";state.selected=null;state.openMenu=null;state.sortMenu=false;toast("Showing keyword: "+tag);render()}
function filterVaultByColor(hex){let color=safeHex(hex);if(!color)return;state.filterHex=color;state.filterKeyword="";state.q="";state.col="all";state.view="vault";state.selected=null;state.openMenu=null;state.sortMenu=false;toast("Showing color: "+color);render()}
function clearTagFilters(){state.filterKeyword="";state.filterHex="";state.q="";if(state.view==="vault")softRefreshVaultResults();else render()}
...
function openMoodboard(boardId){state.activeMoodboard=boardId;state.view="moodboard-edit";state.selectedObject=null;state.selectedObjectIds=[];state.moodboardTool="select";state.moodboardConnectFrom=null;moodboardHistory.clear();history.replaceState(null,"",moodboardAppUrl(boardId));render()}
