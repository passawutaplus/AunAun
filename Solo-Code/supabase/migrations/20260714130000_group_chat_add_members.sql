-- Group chat: any member can add mutual-follow friends
-- Canonical: Solo-Code/supabase/migrations/20260714130000_group_chat_add_members.sql
...
BEGIN
  ...
  -- client_id/freelancer_id are NOT NULL; for groups both point at creator
...
BEGIN
  ...
  FOREACH _member IN ARRAY _all_members LOOP
    ...
    -- Mutual follow required between inviter and invitee
    IF NOT (
      EXISTS (
      ...
      AND EXISTS (
        SELECT 1 FROM anthem.follows f
        WHERE f.follower_id = _member AND f.following_id = _uid
      )
    ) THEN
