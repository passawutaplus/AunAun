...
const APLUS1_APP_URL = APLUS1_SHOWCASE_URL.replace(/\/$/, "");
...
export function Aplus1JobsPanel({ onOpenQuotations }: { onOpenQuotations?: () => void }) {
  const { user } = useAuth();

  const { data: hires = [], isLoading } = useQuery({
    queryKey: ["aplus1-hiring-requests", user?.id],
    enabled: !!user?.id,
    queryFn: async () => {
            ...
            >
              ...
              <div className="flex flex-col gap-1 shrink-0">
                ...
                <Button type="button" size="sm" variant="ghost" className="h-7 text-xs" asChild>
                  <a
                    href={`${APLUS1_APP_URL}/portfolio/manage`}
                    target="_blank"
                    rel="noopener noreferrer"
                  >
...
/** @deprecated use Aplus1JobsPanel */
export const AnthemJobsPanel = Aplus1JobsPanel;
